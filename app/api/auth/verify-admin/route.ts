...
function safeEqual(a: string, b: string): boolean {
  ...
  if (ba.length !== bb.length) {
    // Сравниваем с фиктивным значением, чтобы не раскрывать длину пароля по времени.
...
export async function POST(req: Request) {
  ...
  if (scope === "staff") {
    ...
    if (!safeEqual(password, expected)) return fail("Неверный пароль для входа в панель.", 401)
  ...
  if (!safeEqual(password, expected)) return fail("Неверный пароль администратора.", 401)
