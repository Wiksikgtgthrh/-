...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < normalized.length; i++) {
    ...
    const values = {
      ...
      allergens: String(r.allergens ?? ""),
      additives: String(r.additives ?? ""),
      shelfLife: String(r.shelf_life ?? ""),
      storageConditions: String(r.storage_conditions ?? ""),
      regulatoryDocuments: String(r.regulatory_documents ?? ""),
