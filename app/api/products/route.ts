...
export async function POST(req: NextRequest) {
  ...
  if (imageFile instanceof File && imageFile.size > 0) {
    try {
      ...
      return fail(e instanceof Error ? e.message : "Не удалось сохранить изображение.")
    }
  }
  ...
  const inserted = await db
    ...
    .values({
      ...
      additives: String(form.get("additives") || ""),
      shelfLife: String(form.get("shelf_life") || ""),
      storageConditions: String(form.get("storage_conditions") || ""),
      regulatoryDocuments: String(form.get("regulatory_documents") || ""),
